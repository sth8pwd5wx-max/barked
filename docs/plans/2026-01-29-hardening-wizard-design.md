...
Two self-contained scripts — `harden.sh` (macOS + Linux) and `harden.ps1` (Windows) — that walk a user through security hardening with a wizard interface. Quick profile selection for fast deployment, advanced questionnaire for custom configuration.
...
Distribution: clone repo or download single script file. `chmod +x harden.sh && sudo ./harden.sh`.
