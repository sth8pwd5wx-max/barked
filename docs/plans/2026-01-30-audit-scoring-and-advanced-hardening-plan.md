...
**Goal:** Add hardening score, severity ratings, audit mode, dry-run mode, non-interactive mode, pre-change analysis, and three advanced hardening modules with mandatory vetting to `scripts/harden.sh`.

**Architecture:** All changes go into the single `scripts/harden.sh` file (~3164 lines). New globals and severity map are added near existing globals (line ~35). New functions slot into existing sections. The module runner gains a dry-run guard. Three new modules + reverts are added after the existing 25 modules. The argument parser and main flow are extended.
...
**Files:**
- Modify: `scripts/harden.sh:35-66` (GLOBALS section)
...
**Files:**
- Modify: `scripts/harden.sh:3060-3088` (parse_args function)
...
**Files:**
- Modify: `scripts/harden.sh` — insert new section after LOGGING & OUTPUT UTILITIES (after line ~170, before OS DETECTION)
...
**Files:**
- Modify: `scripts/harden.sh` — insert after the scoring section, before OS detection
...
**Files:**
- Modify: `scripts/harden.sh` — add `run_audit()` function before the MAIN section (~line 3090), and update `main()` to route to it
...
**Files:**
- Modify: `scripts/harden.sh` — update `main()` harden flow (lines ~3130-3141)
...
**Files:**
- Modify: `scripts/harden.sh` — update `run_module()` and add dry-run report logic
...
**Files:**
- Modify: `scripts/harden.sh` — update `main()` harden flow
...
**Files:**
- Modify: `scripts/harden.sh` — add `vet_advanced_module()` and integrate into module runner
...
**Files:**
- Modify: `scripts/harden.sh` — add modules after `mod_bluetooth_disable` (~line 2253), add reverts after existing reverts (~line 2766)
...
# Security hardening — applied by harden.sh
kernel.randomize_va_space = 2
fs.suid_dumpable = 0
net.ipv4.conf.all.rp_filter = 1
net.ipv4.tcp_syncookies = 1
...
# Security hardening — added by harden.sh
...
**Files:**
- Modify: `scripts/harden.sh` — update `print_summary()` (line ~2944)
...
**Files:**
- Modify: `scripts/harden.sh` — update header comment, version
